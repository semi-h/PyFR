...
<%pyfr:macro name='viscous_flux_buffadd' params='uin, grad_uin, fout'>
    fpdtype_t rho = uin[0], rhou = uin[1], rhov = uin[2], E = uin[3];

    fpdtype_t rcprho = 1.0/rho;
    fpdtype_t u = rcprho*rhou, v = rcprho*rhov;

    fpdtype_t rho_x = grad_uin[(0*_ny+_y)*BLK_SZ*4 + X_IDX_AOSOA(0, 4)];//[0][0];
    fpdtype_t rho_y = grad_uin[(1*_ny+_y)*BLK_SZ*4 + X_IDX_AOSOA(0, 4)];//[1][0];

    // Velocity derivatives (rho*grad[u,v])
    fpdtype_t u_x = grad_uin[(0*_ny+_y)*BLK_SZ*4 + X_IDX_AOSOA(1, 4)] - u*rho_x;
    fpdtype_t u_y = grad_uin[(1*_ny+_y)*BLK_SZ*4 + X_IDX_AOSOA(1, 4)] - u*rho_y;
    fpdtype_t v_x = grad_uin[(0*_ny+_y)*BLK_SZ*4 + X_IDX_AOSOA(2, 4)] - v*rho_x;
    fpdtype_t v_y = grad_uin[(1*_ny+_y)*BLK_SZ*4 + X_IDX_AOSOA(2, 4)] - v*rho_y;

    fpdtype_t E_x = grad_uin[(0*_ny+_y)*BLK_SZ*4 + X_IDX_AOSOA(3, 4)];//[0][3];
    fpdtype_t E_y = grad_uin[(1*_ny+_y)*BLK_SZ*4 + X_IDX_AOSOA(3, 4)];//[1][3];
...
<%pyfr:macro name='viscous_flux_buffqadd' params='uin, grad_uin, fout'>
    fpdtype_t rho  = uin[BLK_SZ*4*_y + X_IDX_AOSOA(0, 4)];
    fpdtype_t rhou = uin[BLK_SZ*4*_y + X_IDX_AOSOA(1, 4)];
    fpdtype_t rhov = uin[BLK_SZ*4*_y + X_IDX_AOSOA(2, 4)];
    fpdtype_t E    = uin[BLK_SZ*4*_y + X_IDX_AOSOA(3, 4)];
...
<%pyfr:macro name='viscous_flux_buffadd' params='uin, grad_uin, fout'>
    ...
    fpdtype_t rho_x = grad_uin[(0*_ny+_y)*BLK_SZ*5 + X_IDX_AOSOA(0, 5)];//[0][0];
    fpdtype_t rho_y = grad_uin[(1*_ny+_y)*BLK_SZ*5 + X_IDX_AOSOA(0, 5)];//[1][0];
    fpdtype_t rho_z = grad_uin[(2*_ny+_y)*BLK_SZ*5 + X_IDX_AOSOA(0, 5)];//[2][0];

    // Velocity derivatives (rho*grad[u,v,w])
    fpdtype_t u_x = grad_uin[(0*_ny+_y)*BLK_SZ*5 + X_IDX_AOSOA(1, 5)] - u*rho_x;
    fpdtype_t u_y = grad_uin[(1*_ny+_y)*BLK_SZ*5 + X_IDX_AOSOA(1, 5)] - u*rho_y;
    fpdtype_t u_z = grad_uin[(2*_ny+_y)*BLK_SZ*5 + X_IDX_AOSOA(1, 5)] - u*rho_z;
    fpdtype_t v_x = grad_uin[(0*_ny+_y)*BLK_SZ*5 + X_IDX_AOSOA(2, 5)] - v*rho_x;
    fpdtype_t v_y = grad_uin[(1*_ny+_y)*BLK_SZ*5 + X_IDX_AOSOA(2, 5)] - v*rho_y;
    fpdtype_t v_z = grad_uin[(2*_ny+_y)*BLK_SZ*5 + X_IDX_AOSOA(2, 5)] - v*rho_z;
    fpdtype_t w_x = grad_uin[(0*_ny+_y)*BLK_SZ*5 + X_IDX_AOSOA(3, 5)] - w*rho_x;
    fpdtype_t w_y = grad_uin[(1*_ny+_y)*BLK_SZ*5 + X_IDX_AOSOA(3, 5)] - w*rho_y;
    fpdtype_t w_z = grad_uin[(2*_ny+_y)*BLK_SZ*5 + X_IDX_AOSOA(3, 5)] - w*rho_z;

    fpdtype_t E_x = grad_uin[(0*_ny+_y)*BLK_SZ*5 + X_IDX_AOSOA(4, 5)];
    fpdtype_t E_y = grad_uin[(1*_ny+_y)*BLK_SZ*5 + X_IDX_AOSOA(4, 5)];
    fpdtype_t E_z = grad_uin[(2*_ny+_y)*BLK_SZ*5 + X_IDX_AOSOA(4, 5)];
...
<%pyfr:macro name='viscous_flux_buffqadd' params='uin, grad_uin, fout'>
    fpdtype_t rho  = uin[BLK_SZ*5*_y + X_IDX_AOSOA(0, 5)];
    fpdtype_t rhou = uin[BLK_SZ*5*_y + X_IDX_AOSOA(1, 5)];
    fpdtype_t rhov = uin[BLK_SZ*5*_y + X_IDX_AOSOA(2, 5)];
    fpdtype_t rhow = uin[BLK_SZ*5*_y + X_IDX_AOSOA(3, 5)];
    fpdtype_t E    = uin[BLK_SZ*5*_y + X_IDX_AOSOA(4, 5)];
