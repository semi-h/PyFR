...
//d for scal_fpts, c for vec_ftps uout for scal_upts_out, uu
...
% for i, j in pyfr.ndrange(ndims, nvars):
    ${(f'buffout[({i}*_ny+_y)*BLK_SZ*{nvars} + X_IDX_AOSOA({j}, {nvars})]')} =
...
% else:
    ...
    fpdtype_t ftemp[${ndims}][${nvars}];
    fpdtype_t p, v[${ndims}];
    ${pyfr.expand('inviscid_flux_buffadd', 'buffuq', 'ftemp', 'p', 'v')};
    ${pyfr.expand('viscous_flux_buffqadd', 'buffuq', 'buffqout', 'ftemp')};
    ${pyfr.expand('artificial_viscosity_add', 'buffqout', 'ftemp', 'artvisc')};
    ...
    // Transform the fluxes
% for i, j in pyfr.ndrange(ndims, nvars):
    ${(f'buffqout[({i}*_ny+_y)*BLK_SZ*{nvars} + X_IDX_AOSOA({j}, {nvars})]')} =
        ${' + '.join(f'smats[{i}][{k}]*ftemp[{k}][{j}]' for k in range(ndims))};
% endfor
% endif
</%pyfr:kernel>
